        ...
        if im1 != im2:
            ...
            for mode in ['euclidean', 'similarity', 'affine', 'projective']:
                ...
                if img1 is not None and img2 is not None:
                    ...
                    # Reprojection Error
                    invH = np.linalg.inv(finalH.params)/np.linalg.inv(finalH.params)[2,2]
                    R = []
                    for corr in correspondenceList:
                        # print (corr) # y1-corr[0] x1-corr[1] y2-corr[2] x2-corr[3]
                        a = corr [0] - np.divide (finalH.params[0,0]*corr[2] + finalH.params[0,1]*corr[3] +finalH.params[0,2], 
                                                  finalH.params[2,0]*corr[2] + finalH.params[2,1]*corr[3] +1)
                        b = corr[1] - np.divide(finalH.params[1,0]*corr[2]+finalH.params[1,1]*corr[3]+finalH.params[1,2],
                                                finalH.params[2,0]*corr[3]+finalH.params[2,1]*corr[2]+1)
                    ...
                    dst = cv2.warpPerspective(img1,finalH.params,(cols,rows))
